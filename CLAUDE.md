...
- `bestand_pct`-Modellwechsel (Crookston & Stage 1999, projizierte Kronendeckung): `bestand_pct` ist Teil von `lst.parquet`. Der Lazy-Add in `load_lst()` recomputet **nur fehlende** Spalten — ein alter Cache mit naivem `bestand_pct` wird **nicht** automatisch korrigiert. Nach Änderung von `_compute_bestand_pct` daher **zwingend `lst.parquet` löschen / `?refresh=true`**.
