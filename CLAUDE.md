...
- `_compute_seal_pct(lst_gdf)` – flächengewichteter Versiegelungsgrad (0–1) je LST-Kachel aus den Entsiegelungs-Polygonen (`load_entsiegelung()`). `gpd.overlay(..., how="intersection")`, Flächen metrisch in EPSG:25832. **Priority-Union** (`_SEAL_MODEL_VERSION = 2`): ATKIS- und OSM-Polygone überlappen sich (concat, kein Verschnitt) — jeder m² zählt genau einmal, zur höchsten `SEAL_RATE_BY_TYPE`-Rate der ihn überdeckenden Polygone (Rate-Gruppen absteigend, `difference()` gegen bereits beanspruchte Fläche). Verhindert die Doppelzählung, die stark versiegelte Zellen (Marktplatz, Talavera) vorher fälschlich auf 100 % klemmte → 0 pflanzbare m². `dominant_type_key` = Kategorie mit größter Überlappung (nur fürs Label). Kacheln ohne Polygon → `seal=0`, `dominant=None` (E2: Abwesenheit von sie02/ver01 ≈ Grün-/Freifläche). Gibt `(seal_pct, dominant_type_key)` zurück.
