...
- `SimEntsiegelungLayer.jsx` (`overlays/`) – deck.gl GeoJsonLayer auf Entsiegelungs-Polygone; selektierte Polygone (`selectedPolygons`) farblich hervorgehoben, Flachdächer (`osm_flat_roof_industrial`) nicht pickable. `onPolygonClick` gibt `{ type_key, area_m2, index }` zurück. Wird auf `/simulation` (Tab Entsiegelung) verwendet.
...
- `simulation/WasserSimPanel.jsx` – Simulation-Panel für Entsiegelung. Liest `selectedPolygons` + `selectedPolygonsAreaM2` aus Store. Gruppiert Polygone nach `type_key`, zeigt Σ Fläche + Versiegelungsgrad je Gruppe. Von/Zu-Belagstyp-Selects pro Gruppe (gesteuert über `groupConfig` im Store). Proportionale Flächenaufteilung über mehrere API-Calls (`Promise.all`). Ergebnis-Grid: Versickerung (m³/Jahr) / Retention (%) / **Anschaulicher Wasserkontext** via `formatWasserKontext()`: Eimer à 10 L (< 2 m³/J), Badewannen à 150 L (< 150 m³/J), Schwimmbecken à 50 m³ (≥ 150 m³/J).
...
- `store/useAppStore.js` – `layers`: `heatmap` (default true), `trees`, `zensus` (default false — Demografie-Layer auf `/vulnerabilitaet`), `vulnerabilitaet`, `entsiegelung_atkis` (default true), `entsiegelung_osm` (default true), `stadtbezirke`; `vulnWeights` (null | object), `setVulnWeights()`; `sim`-Slice: `selectedCells` (Index-Array), `selectedCellsAreaM2`, `selectedPolygons` (`[{ type_key, area_m2, index }]`), `selectedPolygonsAreaM2`, `showBaumkataster` (bool), `baeume.anzahl` (int), `wasser.flaeche_m2` (int), `wasser.groupConfig` (`{ [type_key]: { from_surface, to_surface } }`); Actions: `toggleSimCell`, `toggleSimPolygon`, `clearSimCells`, `clearSimPolygons`, `toggleSimBaumkataster`, `setSimBaeumeAnzahl`, `setSimWasserFlaeche`, `setSimWasserGroupSurface`
...
- `utils/simulate.js` – `CROWN_AREA_M2_DEFAULT` (25 m²), `SEAL_RATE` (Versiegelungsgrade je `type_key`), `getSealRate(type_key)`, `SURFACE_LABELS` (Belagstyp-Anzeigenamen), `TYPE_KEY_LABELS` (ATKIS/OSM-Flächentyp-Labels), `getFromSurface(type_key)` (Default-Ausgangsbelag je Flächentyp)
