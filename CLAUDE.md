...
| `GET /api/simulate/baeume` | ✅ | Neupflanzungen → Δ°C LST + CO₂-Bindung + Kronendeckung. Params: `n_trees`, `area_m2`, `existing_coverage_pct` (optional, default 0). Headroom-aware: Δ°C wirkt nur auf verbleibenden Deckungsraum. Koeffizient `LST_PER_PCT_CANOPY_MIXED` (−0,083 °C/%-Punkt, München). Response: `delta_lst_celsius`, `co2_kg_year`, `delta_coverage_pct`, `effective_new_pct`, `total_coverage_pct`, `coefficients_used`, `caveats`. |
...
- `simulation/BaumSimPanel.jsx` – Simulation-Panel für Baumpflanzung. Liest `selectedCells` + `selectedCellsAreaM2` aus Store. Berechnet `existingPct` (Ø Kronendeckung selektierter Zellen aus `bestand_pct`-Property) und `avgLstCelsius` (Ø LST der selektierten Kacheln), headroom-aware Slider-Max. Debounced API-Call zu `/api/simulate/baeume`. **Slider + editierbares Zahleneingabefeld** (synchron via `inputVal`-State; Blur/Enter klemmt auf [1, sliderMax]). **Kronendeckungs-Stacked-Bar** (Bestand / Neu / Headroom als farbige Segmente, löst Fließtext-Hinweis ab). **Vorher/Nachher-Block** mit inliner `BeforeAfterRow`-Komponente: drei Zeilen (Temperatur LST / CO₂-Bindung / Kronendeckung) je mit Progress-Bar, Ghost-Füllung für Ausgangszustand, animiertem Balken auf Nachher-Wert, Delta-Badge rechts. Methodik-Toggle mit Caveats + Koeffizienten.
