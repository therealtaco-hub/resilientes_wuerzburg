...
Enthält: `LST_PER_PCT_CANOPY_OVERALL` (−0,069 °C/%), `LST_PER_PCT_CANOPY_MIXED` (−0,083 °C/%), `LST_PER_PCT_CANOPY_RECREATIONAL` (−0,038 °C/%), `CROWN_AREA_M2_DEFAULT` (50 m²), `TRANSPIRATION_LB3/LB6_KG_M2_DAY`, `LST_PER_PCT_UNSEALING` (−0,03 °C/%), `RUNOFF_COEFFICIENTS` (Ψ-Werte je Belagstyp, 9 Klassen, DWA-A138 / Bayreuth 2024), `ANNUAL_RAINFALL_WUERZBURG_M` (0,5735 m/Jahr, DWD Station 05705), `MONTHLY_RAINFALL_WUERZBURG_MM`, `CO2_KG_PER_TREE_YEAR` (12,5 kg), `SEAL_RATE_BY_TYPE` (Versiegelungsgrade je ATKIS/OSM-Typ, Literaturwerte).
...
- Wissenschaftliche Dokumentation: `docs/Tool-Logik-und-Quellen.md` / `.html` — Tab-für-Tab-Beschreibung aller Funktionen, Datenquellen und Koeffizienten. Aktualisieren nach größeren Feature-Änderungen.
...
| `GET /api/lst` | ✅ | GeoTIFF (`lst_wue_2023_2025_summer_median.tif`, EPSG:3035, 100m, Zensus-aligned). ~14.500 Features. Properties: `lst_celsius` (°C, 1 Dez.), `lst_norm` (Rang-normiert 0–1), `bestand_pct` (projizierte Kronendeckung), `seal_pct` (flächengew. Versiegelungsgrad 0–1), `dominant_type_key` (dominante ATKIS/OSM-Kategorie je Zelle, `null` bei keiner Überdeckung), `plantable_m2` (`10.000 × (1−seal_pct)`). Cache: `backend/data/lst.parquet`. `?refresh=true` erzwingt Neuberechnung. |
...
| `GET /api/stadtbezirke` | ✅ | 13 Würzburger Stadtbezirke als GeoJSON FeatureCollection mit aggregierten Kennzahlen. Quelle: opendata.wuerzburg.de API (`stadtbezirke`-Datensatz). Properties: `name`, `nummer`, `lst_max`, `lst_median`, `lst_mean`, `hvi_max`, `hvi_mean` (**bevölkerungsgewichteter** Durchschnitt: Σ(hvi·ew)/Σ(ew), Fallback auf ungewichteten Mean), `einwohner` (Σ aus Zensus-sjoin), `entsiegelung_m2` (Σ ATKIS+OSM-Flächen), `tree_count`. Spatial Joins gegen LST-Pixel, HVI-Zellen, Entsiegelung-Polygone, Baumkataster. `meta.total_count`. Cache: `backend/data/stadtbezirke.parquet`. `?refresh=true` lädt API neu. |
| `GET /api/simulate/baeume` | ✅ | Neupflanzungen → Δ°C LST + CO₂-Bindung + Kronendeckung. Params: `n_trees`, `area_m2`, `existing_coverage_pct` (optional, default 0). **Projizierte Kronendeckung (Überlappungsmodell, Crookston & Stage 1999):** Bestand & Neu werden im Flächen-Verhältnis addiert (`existing_ratio = −ln(1−existing/100)`, `total = (1−exp(−(existing_ratio+new_ratio)))·100`), Δ°C wirkt nur auf den projizierten Zuwachs `effective_new_pct` (≥ 0, kein Headroom-Cap mehr). Koeffizient `LST_PER_PCT_CANOPY_MIXED` (−0,083 °C/%-Punkt, München). Response: `delta_lst_celsius`, `co2_kg_year`, `crown_area_ratio` (Modell-Input, ersetzt das entfernte `delta_coverage_pct`), `effective_new_pct`, `total_coverage_pct`, `coefficients_used`, `caveats`. |
| `GET /api/simulate/wasser` | ✅ | Entsiegelungsfläche → m³ Versickerung/Jahr (Rational-Formel). Params: `area_m2`, `from_surface`, `to_surface` (Belagstypen aus `RUNOFF_COEFFICIENTS`). Response: `infiltration_m3_year`, `retention_pct`, `context_persons` (150 L/Tag × 365), `runoff_coefficients`, `rainfall_m_year`, `caveats`. 422 bei unbekannten Belagstypen. |
...
- `load_lst(force_refresh=False)` – liest `lst_wue_2023_2025_summer_median.tif` (EPSG:3035, 100m, bereits auf Destatis-Gitter gesnappt). Kein Resampling, keine cos(lat)-Korrektur. Erstellt Pixel-Bounding-Boxes direkt aus rasterio-Transform. Speichert `x_mp_100m`/`y_mp_100m` (Integer-Mittelpunkte in EPSG:3035) als Merge-Schlüssel. `lst_celsius` direkt aus Band 1 (GEE exportiert °C). `lst_norm` via `scipy.stats.rankdata`. Reichert `bestand_pct` (via `_compute_bestand_pct`) + `seal_pct`/`dominant_type_key` (via `_compute_seal_pct`) an. Lazy-Add für alte Caches: fehlende Spalten werden nachgereicht (gilt **nicht** für bereits vorhandene, aber inhaltlich veraltete Spalten — dann `?refresh=true`). Cache: `backend/data/lst.parquet`.
- `_compute_seal_pct(lst_gdf)` – flächengewichteter Versiegelungsgrad (0–1) je LST-Kachel aus den Entsiegelungs-Polygonen (`load_entsiegelung()`). `gpd.overlay(..., how="intersection")`, Überlappungsfläche metrisch in EPSG:25832, `seal_pct = Σ(overlap_area × SEAL_RATE_BY_TYPE[type_key]) / 10.000`, geklemmt auf [0,1] (fängt Polygon-Doppelzählung ab). `dominant_type_key` = Kategorie mit größter Überlappung (nur fürs Label). Kacheln ohne Polygon → `seal=0`, `dominant=None` (E2: Abwesenheit von sie02/ver01 ≈ Grün-/Freifläche). Gibt `(seal_pct, dominant_type_key)` zurück.
...
| Vulnerabilität | `/vulnerabilitaet` | ✅ inkl. HVI-Legende + LST-Legende + Demografie-Legende + Stadtbezirks-Choropleth (HVI) |
| Entsiegelung | `/entsiegelung` | ✅ |
| Simulation | `/simulation` | ✅ Tab-Layout (Baumpflanzung / Entsiegelung), Karten-Selektion, Slider, Ergebnis-Grid, Methodik-Toggle |
...
- `StadtbezirkeVulnLayer.jsx` (`overlays/`) – deck.gl GeoJsonLayer-Choropleth auf `hvi_max` für `/vulnerabilitaet`. Lila-Gradient: Alpha dynamisch normiert auf Bezirksspanne (30–180), `[168,85,247,alpha]`. `pickable: true`, akzeptiert `onHover`-Prop. `updateTriggers: { getFillColor: [range.min, range.max] }`.
- `StadtbezirkeHviLegend.jsx` – Frosted-glass-Legende analog `StadtbezirkeLegend`, aber Lila-Gradient + `fmt.index()`-Labels (min/median/max), Titel „Stadtbezirke · HVI max". Props: `{ min, median, max }`.
- `EntsiegelungLayer.jsx` (`overlays/`) – **ein** GeoJsonLayer für alle Features; `getFillColor` switcht per `type_key` auf feste RGBA-Farbe (Alpha 170); Filterung per `showAtkis`/`showOsm` vor Layer-Erstellung. Kein seal_rate. OSM-Cases: `osm_parking`, `osm_square`, `osm_flat_roof_industrial` (hellgrün `[134,239,172,170]`). Optionale Props `pickable` (default true) + `alpha` (überschreibt den 4. Farbkanal) — auf `/simulation` (Baum-Tab) als gedimmtes, nicht-pickbares Versiegelungs-Overlay wiederverwendet.
...
- `SimCellLayer.jsx` (`overlays/`) – deck.gl GeoJsonLayer auf LST-Daten; selektierte Zellen (`selectedCells` als Index-Set) grün hervorgehoben, pickable; `onCellClick` gibt Feature-Index zurück. Wird auf `/simulation` (Tab Baumpflanzung) verwendet.
- `SimEntsiegelungLayer.jsx` (`overlays/`) – deck.gl GeoJsonLayer auf Entsiegelungs-Polygone; selektierte Polygone (`selectedPolygons`) farblich hervorgehoben, Flachdächer (`osm_flat_roof_industrial`) nicht pickable. `onPolygonClick` gibt `{ idx, type_key, area_m2, label }` zurück. Wird auf `/simulation` (Tab Entsiegelung) verwendet.
- `simulation/BaumSimPanel.jsx` – Simulation-Panel für Baumpflanzung. Liest `selectedCells` + `selectedCellsAreaM2` aus Store. Berechnet `existingPct` (Ø Kronendeckung selektierter Zellen aus `bestand_pct`-Property) und `avgLstCelsius` (Ø LST der selektierten Kacheln). **Slider-Max = pflanzpraktischer Cap auf die *pflanzbare* Fläche** (`floor(plantableAreaM2 / MIN_GROUND_PER_TREE_M2)`, 100 m²/Baum — FLL-Richtlinie, Bäume 2. Ordnung; `plantableAreaM2 = selectedCellsAreaM2 × (1 − avgSealPct)` aus `seal_pct`-Property — kein Modell-Cap). `area_m2` an die API bleibt die **volle** Fläche (Kronen überhängen versiegelten Boden). **Versiegelungs-Readout** im Banner: dominante Kategorie (`dominant_type_key` → `TYPE_KEY_LABELS`), `~X % versiegelt`, pflanzbare Fläche, max Bäume; Lücken-Hinweis bei `dominant_type_key == null` (E2). **LST-Toggle** (`showSimLst`) blendet die LST-Farbcodierung (HeatLayer) aus — das 100×100-Raster bleibt über `SimCellLayer` sichtbar/klickbar (verhindert Farbchaos mit dem ATKIS-Overlay); die LST-Legende im Panel wird dann mit ausgeblendet. **ATKIS-Overlay-Toggle** (`showSimAtkis`) blendet die Versiegelungspolygone als Verifikations-Layer ein. `totalPct`/`effectiveNewPct` spiegeln das Backend-Überlappungsmodell (Crookston & Stage 1999) für die Slider-Vorschau. Debounced API-Call zu `/api/simulate/baeume`. **Slider + editierbares Zahleneingabefeld** (synchron via `inputVal`-State; Blur/Enter klemmt auf [1, sliderMax]). **Kronendeckungs-Stacked-Bar** (Bestand / Neu / Frei als farbige Segmente). Überlappungs-Hinweis, wenn neue Kronen überwiegend auf bereits beschatteten Boden fallen. **Vorher/Nachher-Block** mit inliner `BeforeAfterRow`-Komponente: drei Zeilen (Temperatur LST / CO₂-Bindung / Kronendeckung) je mit Progress-Bar, Ghost-Füllung für Ausgangszustand, animiertem Balken auf Nachher-Wert, Delta-Badge rechts. Methodik-Toggle mit Caveats + Koeffizienten.
- `simulation/WasserSimPanel.jsx` – Simulation-Panel für Entsiegelung. Liest `selectedPolygons` + `selectedPolygonsAreaM2` aus Store. Gruppiert Polygone nach `type_key`. **Panel-Aufbau:**
  1. **Flächenaufschlüsselung** mit drei `AreaRow`-Zeilen (Gesamt / Davon versiegelt / Typ. entsiegelbar) je mit Fortschrittsbalken
  2. **Belagstypen-Gruppen** pro type_key: Versiegelungsgrad-Balken, Von-Select + `PsiChip` (rot/amber/grün je Ψ) + Delta-Chip (↓ Ψ−X · Verbesserung) + Zu-Select + `PsiChip`. Option-Labels zeigen `(Ψ 0.90)` direkt im nativen Select
  3. **Slider + Texteingabe** synchron (wie BaumSimPanel), Hint unter Slider: max. versiegelte Σ-Fläche (grau) + amber-Hinweis "Typisch realistisch: ~X m²" (Schätzwert, kein harter Cap)
  4. **Ergebnisse**: `WasserBalanceBar` Vorher/Nachher (Abfluss rot + Versickerung blau, 7px-Balken), großer Versickerungswert (m³/Jahr) + ≈N Personen-Jahrestrinkwasserbedarf (BDEW 2023, 127 L/Tag) + anschaulicher Kontext via `formatWasserKontext()` (Eimer/Badewannen/Schwimmbecken), **Grundwasserneubildungs-Box** mit ~15–30 % Richtwert (LfU Bayern) + amber Caveat
  5. **Methodik-Toggle** mit Berechnungsformel, Ψ-Tabelle (DWA-A138/Bayreuth 2024), Versiegelungsgrad-Tabelle, Realisierbarkeitsfaktoren-Tabelle
  
  Konstanten: `WATER_USE_M3_PER_PERSON = 46,4` (BDEW 2023), `GW_RATE_LOW = 0,15` / `GW_RATE_HIGH = 0,30` (LfU Bayern). Inline-Komponenten: `PsiChip`, `AreaRow`, `WasserBalanceBar`. `avgFromPsiStatic`: flächengewichteter Ψ-Vorher aus groupConfig (ohne API-Call). API-Calls proportional aufgeteilt via `Promise.all`, collectiert `avgFromPsi`/`avgToPsi` für Balken.
- `simulation/SimResultCard.jsx` – Einzelne Ergebnis-Kachel. Props: `{ label, value, unit?, color, caveat?, loading, empty }`. Loading-Spinner, leerer Zustand (gestrichelt), Wert + optionaler Caveat-Hinweis. Wird weiterhin im WasserSimPanel verwendet.
...
- `store/useAppStore.js` – `sidebarCollapsed` (bool, default false), `toggleSidebar()`; `layers`: `heatmap` (default true), `trees`, `zensus` (default false — Demografie-Layer auf `/vulnerabilitaet`), `vulnerabilitaet`, `entsiegelung_atkis` (default true), `entsiegelung_osm` (default true), `stadtbezirke`, `ndvi` (default false); `layerLoading` (map), `setLayerLoading(key, bool)`; `vulnWeights` (null | object), `setVulnWeights()`; `sim`-Slice: `selectedCells` (Index-Array), `selectedCellsAreaM2`, `selectedPolygons` (`[{ idx, type_key, area_m2, label }]`), `selectedPolygonsAreaM2`, `showBaumkataster` (bool), `showSimLst` (bool, default true — LST-Farbcodierung/HeatLayer im Baum-Tab; aus = nur Raster via SimCellLayer sichtbar), `showSimAtkis` (bool, default false — ATKIS/OSM-Versiegelungs-Overlay im Baum-Tab), `baeume.anzahl` (int), `wasser.flaeche_m2` (int, gecappt auf Σ sealable bei Auswahl-Änderung), `wasser.groupConfig` (`{ [type_key]: { from_surface, to_surface } }` — auto-befüllt beim ersten Polygon je type_key); Actions: `toggleSimCell`, `toggleSimPolygon`, `clearSimCells`, `clearSimPolygons`, `toggleSimBaumkataster`, `toggleSimLst`, `toggleSimAtkis`, `setSimBaeumeAnzahl`, `setSimWasserFlaeche`, `setSimWasserGroupSurface`. Hinweis: `_baeumeSliderMax` cappt `anzahl` grob auf `floor(Fläche/100)` (volle Fläche, `_MIN_GROUND_PER_TREE_M2 = 100`); der seal-aware Finalclamp (`floor(pflanzbare Fläche/100)`) passiert im `BaumSimPanel` (kennt `seal_pct`).
...
- `api/simulate.js` – `fetchSimulateBaeume({ n_trees, area_m2, existing_coverage_pct })` + `fetchSimulateWasser({ area_m2, from_surface, to_surface })`
- `utils/simulate.js` – Gespiegelte Simulation-Konstanten (sync mit `backend/simulation_params.py`). Exports: `CROWN_AREA_M2_DEFAULT` (50 m²), `MIN_GROUND_PER_TREE_M2` (100 m²/Baum — pflanzpraktischer Slider-Cap nach FLL-Richtlinie für Bäume 2. Ordnung, kein Modell-Cap), `SEAL_RATE` (Versiegelungsgrade je `type_key`), `getSealRate(type_key)`, `FROM_SURFACE_BY_TYPE_KEY` (Default-Ausgangsbelag je Flächentyp), `getFromSurface(type_key)`, `SURFACE_LABELS` (Belagstyp-Anzeigenamen), `SURFACE_ORDER` (absteigend nach Versiegelung, 9 Stufen), `getNextBetterSurface(from)`, `isAlreadyGreenest(from)`, `TYPE_KEY_LABELS` (ATKIS/OSM-Flächentyp-Labels), `RUNOFF_COEFFICIENTS` (Ψ je Belagstyp, gespiegelt aus simulation_params, DWA-A138 / Bayreuth 2024), `TYPICAL_REALIZATION_RATE` (typisch entsiegelbarer Anteil je Flächentyp — informativer Hinweis, kein harter Cap), `getTypicalRealizationRate(type_key)`
...
- `bestand_pct`-Modellwechsel (Crookston & Stage 1999, projizierte Kronendeckung) **und** `seal_pct`/`dominant_type_key` (Versiegelungsgrad): beide sind Teil von `lst.parquet`. Der Lazy-Add in `load_lst()` recomputet **nur fehlende** Spalten — ein alter Cache mit naivem `bestand_pct` wird **nicht** automatisch korrigiert. Nach Änderung von `_compute_bestand_pct`/`_compute_seal_pct` daher **zwingend `lst.parquet` löschen / `?refresh=true`** (neue Spalten wie `seal_pct` werden zwar lazy ergänzt, aber inhaltliche Änderungen an bestehenden Spalten nicht).
...
- [x] Stadtbezirks-Choropleth auf der Vulnerabilitäts-Seite (`StadtbezirkeVulnLayer`, Lila-Gradient auf `hvi_max`). `hvi_mean` im Backend auf bevölkerungsgewichteten Durchschnitt umgestellt (Σ hvi·ew / Σ ew), im Tooltip als „ew-gew." gekennzeichnet.
...
- [ ] Kronenfläche der Bäume berechnen: `kronenfläche_m2 = π × (kronenbrei / 2)²` (Annahme: kreisförmige Krone aus `kronenbrei`-Durchmesser). Verfügbar für 44.647 Bäume. Als Gewicht in Baum-Simulation nutzen: größere Kronen → stärkerer Kühleffekt.
- [x] Simulationsendpoints (Bäume → Δ°C/CO₂; Entsiegelung → m³ Versickerung).
- [x] `/simulation`-Seite vollständig implementiert: Tab-Layout (Baumpflanzung / Entsiegelung), Karten-Selektion (LST-Kacheln / Entsiegelungs-Polygone), Slider + Texteingabe, debounced API-Calls, Vorher/Nachher-Block (LST / CO₂ / Kronendeckung), Kronendeckungs-Stacked-Bar, anschaulicher Wasserkontext (Eimer/Badewanne/Schwimmbecken), Methodik-Toggle mit Caveats.
- [x] **Entsiegelungs-Tab UX-Überarbeitung (WasserSimPanel):** Flächenaufschlüsselung (Gesamt/Versiegelt/Typ.entsiegelbar) mit Fortschrittsbalk, Ψ-Chips + Delta-Chips in Gruppen-Cards, Ψ-Werte direkt in Select-Options, Wasserbalance-Vorher/Nachher-Bars, Grundwasserneubildungs-Schätzwert (15–30 %, LfU Bayern), BDEW-Benchmark (127 L/Tag), Realisierbarkeitsfaktoren als informativer Hinweis (kein harter Cap), Methodik-Toggle mit 4 Abschnitten.
- [x] **Wissenschaftliche Dokumentation** erstellt: `docs/Tool-Logik-und-Quellen.md` + `.html` — Tab-für-Tab-Beschreibung aller Funktionen, Formeln und Datenquellen.
- [ ] Kollabierbare Sidebar (app-weit): Toggle-Button klappt `Sidebar.jsx` von 220px auf ~48px (Icon-only) zusammen. Zustand in `useAppStore` (`sidebarCollapsed`). Design-Referenz: `docs/Design-System.html` prüfen.
- [ ] CI-Integration der Test-Suite (aktuell nur lokal ausführbar).
- [ ] **[Variante A – Quick & Dirty]** Baum-Kühlkoeffizienten aus München vorerst als Default in `backend/simulation_params.py` hinterlegen (globaler Koeffizient −0,069 °C/%-Punkt Baumkronenanteil und nutzungsspezifische Steigungen, Quelle: `urban-heat-wiki/wiki/sources/garcia-de-leon-lst-trees-munich.md`). Im Code alle Baum-Δ°C-Berechnungen ausschließlich über `simulation_params` einbinden und in der Doku klar als „München-Koeffizienten, noch nicht lokal kalibriert" kennzeichnen.
- [ ] **[Variante B – saubere Lösung]** Eigene Baum-Kühlkoeffizienten für Würzburg aus LST-GeoTIFF + Baumkataster ableiten: pro 100m-Zelle Baumkronenanteil berechnen (`kronenfläche_m2` aus `kronenbrei`-Durchmesser), LST vs. Baumanteil global und nach Nutzungsklassen (ATKIS/OSM → Residential/Industrial/Traffic/Mixed/Recreational) linear regressieren, Steigung/Intercept/R² in einer neuen Wiki-Seite (`urban-heat-wiki/wiki/sources/lst-trees-wuerzburg.md`) dokumentieren und die kalibrierten Koeffizienten in `backend/simulation_params.py` übernehmen.